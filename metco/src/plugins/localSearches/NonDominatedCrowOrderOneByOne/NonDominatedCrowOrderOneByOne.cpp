#include "NonDominatedCrowOrderOneByOne.h"
#include "SimpleIndividual.h"
#include "utils.h"

#define ORIGINAL 2
#define SIMPLE 3
void NonDominatedCrowOrderOneByOne::localSearch(vector <Individual*> *p, MOFront *currentSolution){
	...
	while((improved) && (iterations < maxIterations)){
		...
		int inserted = 0;
		for (int i = 0; i < fronts.size(); i++){
			if ((inserted == desiredSize) || (inserted + fronts[i].size() <= desiredSize)){
				for (int j = 0; j < fronts[i].size(); j++){
				...
				while(inserted + fronts[i].size() > desiredSize){
					crowOrder(fronts[i]);
					notClassified.push_back(*(fronts[i].end() - 1));
					fronts[i].pop_back();
				}
				for (int j = 0; j < fronts[i].size(); j++){
					fronts[i][j]->setAlgorithmData(assignedSpaceId, i);
...
bool NonDominatedCrowOrderOneByOne::init(const vector<string> &params){
	if (params.size() != 2){
		cout << "Error, NonDominatedCrowOrderOneByOne parameters --> MaxIterations pLS" << endl;
		return false;				
	}
