...
bool Plugin::init(const vector<string> &params) { return (params.size() == 0); }

// Cargador de plugins
Plugin *loadPlugin(const string &kind, const string &pluginPath,
  ...
  if (hndl == NULL) {
    if (printError) {
      ...
      cerr << "Plugin " << kind << "::" << name
           << " file could not be opened: " << libFich.str() << endl;
    }
    return NULL;
  }
  ...
  if (mkr == NULL) {
    if (printError)
      cerr << "maker function in " << kind << "::" << name << "does no exist"
           << endl;
    return NULL;
  }
  Plugin *newSc = (*mkr)();
  if (newSc == NULL) {
    if (printError)
      cerr << "maker function for " << kind << "::" << name << " returns NULL"
           << endl;
    return NULL;
  }
  if (initialize) {
