...
void PlainText::printSolution(EA *ga, bool end) {
    ...
    if (end) {
        ...
        outputFile << endl
                   << "Time =  " << ga->getElapsedTime() << endl
                   << endl;
    }

    if (ga->getCritStop() == ga->getTypeStoppingCriterion("TIME")) {
        if (end) {
            outputFile << "Current Evaluation = "
        ...
        if (ga->isGeneratingArchive()) {
            ...
            try {
                MOFront *p =
                    new MOFrontVector(ga->getSampleInd(), false, false);
                ga->getSolution(p);
                outputFile << p;
                delete p;
                p = nullptr;
    ...
    } else if (ga->getCritStop() ==
               ga->getTypeStoppingCriterion("EVALUATIONS")) {
        if (end) {
            outputFile << "Current Evaluation = "
                       << ga->getPerformedEvaluations() << endl;
        } else {
