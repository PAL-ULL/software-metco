...
#include "MonoSynergyPairs.h"
#include <math.h>

long double MonoSynergyPairs::score(MOFront *front){
...
long double MonoSynergyPairs::getScore(const long double breakFromScore, const int alg, const int conf, const int islandId, const double rndValue){
	long double imp = 0;
	int n = 0;
...
long double MonoSynergyPairs::estimateScore(const long double breakFromScore, const int alg, const int conf, const int islandId, const double rndValue){
	long double sc = getScore(breakFromScore, alg, conf, islandId, rndValue);
	return sc;
}

void MonoSynergyPairs::updateScore(const long double breakFrom, const long double arriveTo, int algId, int confId, int islandId){
	...
	cout << "Algoritmo: " << algId << ": " << imp << endl << flush;
...
void MonoSynergyPairs::printHistory(){
}

void MonoSynergyPairs::begins(const int algId, const int confId, const int islandId){
...
//n-1 veces.
void MonoSynergyPairs::asynchronyDetection(int islandId){
}

void MonoSynergyPairs::ends(const int algId, const int confId, const int islandId){
	configToExecuting[algId][confId]--;
	asynchronyDetection(islandId); 	
}

//Parámetros: num_islas_esclavas p1(porcentaje de uso puntuacion independiente) p2(porcentaje de uso puntuacion de pares) n(maximos valores media) (0|1) (0: MINIMIZE, 1: MAXIMIZE)
bool MonoSynergyPairs::init(const vector<string> &params) {
	if (params.size() != 5)
		return false;
	...
	optDirection = atoi(params[4].c_str());
	if ((optDirection != 0) && (optDirection != 1)){
		cout << "optDirection must be 0 (MINIMIZE) or 1 (MAXIMIZE)" << endl;
		return false;
	}
	return true;
}
