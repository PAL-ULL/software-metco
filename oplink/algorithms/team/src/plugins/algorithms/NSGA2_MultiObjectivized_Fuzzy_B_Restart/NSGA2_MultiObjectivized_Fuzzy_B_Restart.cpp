...
#include "NSGA2_MultiObjectivized_Fuzzy_B_Restart.h"
#include <float.h>


// Define una generación de búsqueda del algoritmo 
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::runGeneration() {
...
bool NSGA2_MultiObjectivized_Fuzzy_B_Restart::init(const vector<string> &params) {
  ...
  bestPmIn = new fl::InputVariable;
  bestPmIn->setName("BEST_PM_IN");
  bestPmIn->setRange(0.0, 1.0);
  bestPmIn->addTerm(new fl::Triangle("LOW", -0.5, 0, 0.5));
  bestPmIn->addTerm(new fl::Triangle("MEDIUM", 0, 0.5, 1));
  bestPmIn->addTerm(new fl::Triangle("HIGH", 0.5, 1, 1.5));
  fuzzyEngine->addInputVariable(bestPmIn);
  ...
  rules_low->addRule(fl::FuzzyRule::parse("if PM_IN is LOW and FIT is LOW and BEST_PM_IN is LOW then PM_OUT is NEG_LOW", fuzzyEngine));
  rules_low->addRule(fl::FuzzyRule::parse("if PM_IN is LOW and FIT is LOW and BEST_PM_IN is MEDIUM then PM_OUT is POS_LOW", fuzzyEngine));
  rules_low->addRule(fl::FuzzyRule::parse("if PM_IN is LOW and FIT is LOW and BEST_PM_IN is HIGH then PM_OUT is POS_LOW", fuzzyEngine));
  ...
  rules_low->addRule(fl::FuzzyRule::parse("if PM_IN is HIGH and FIT is HIGH then PM_OUT is ZERO", fuzzyEngine));
	...
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if PM_IN is LOW_MED_B and FIT is LOW and BEST_PM_IN is LOW then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if PM_IN is LOW_MED_B and FIT is LOW and BEST_PM_IN is MEDIUM then PM_OUT is POS_LOW", fuzzyEngine));
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if PM_IN is LOW_MED_B and FIT is LOW and BEST_PM_IN is HIGH then PM_OUT is POS_LOW", fuzzyEngine));
	...
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if PM_IN is LOW_MED_A and FIT is LOW and BEST_PM_IN is LOW then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if PM_IN is LOW_MED_A and FIT is LOW and BEST_PM_IN is MEDIUM then PM_OUT is POS_LOW", fuzzyEngine));
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if PM_IN is LOW_MED_A and FIT is LOW and BEST_PM_IN is HIGH then PM_OUT is POS_LOW", fuzzyEngine));
	...
	rules_medium->addRule(fl::FuzzyRule::parse("if PM_IN is MEDIUM and FIT is LOW and BEST_PM_IN is LOW then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_medium->addRule(fl::FuzzyRule::parse("if PM_IN is MEDIUM and FIT is LOW and BEST_PM_IN is MEDIUM then PM_OUT is ZERO", fuzzyEngine));
	rules_medium->addRule(fl::FuzzyRule::parse("if PM_IN is MEDIUM and FIT is LOW and BEST_PM_IN is HIGH then PM_OUT is POS_LOW", fuzzyEngine));
	...
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if PM_IN is MED_HIGH_A and FIT is LOW and BEST_PM_IN is LOW then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if PM_IN is MED_HIGH_A and FIT is LOW and BEST_PM_IN is MEDIUM then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if PM_IN is MED_HIGH_A and FIT is LOW and BEST_PM_IN is HIGH then PM_OUT is POS_LOW", fuzzyEngine));
	...
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if PM_IN is MED_HIGH_B and FIT is LOW and BEST_PM_IN is LOW then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if PM_IN is MED_HIGH_B and FIT is LOW and BEST_PM_IN is MEDIUM then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if PM_IN is MED_HIGH_B and FIT is LOW and BEST_PM_IN is HIGH then PM_OUT is POS_LOW", fuzzyEngine));
	...
	rules_high->addRule(fl::FuzzyRule::parse("if PM_IN is HIGH and FIT is LOW and BEST_PM_IN is LOW then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_high->addRule(fl::FuzzyRule::parse("if PM_IN is HIGH and FIT is LOW and BEST_PM_IN is MEDIUM then PM_OUT is NEG_LOW", fuzzyEngine));
	rules_high->addRule(fl::FuzzyRule::parse("if PM_IN is HIGH and FIT is LOW and BEST_PM_IN is HIGH then PM_OUT is POS_LOW", fuzzyEngine));
...
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::getSolution(MOFront *p) {
...
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::printInfo(ostream &os) const {
	os << "Genetic Algorithm NSGA2_MultiObjectivized_Fuzzy_B_Restart" << endl;
 ...
 * ***********************************************************************/
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::rankCrowdingPopulation() {
	int inserted = 0;
...
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::rankPopulation() {
	vector < vector < Individual * > > fronts;
	vector < Individual *> notClassified;
 ...
 *  - Recombination
 * *****************************************************************************/
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::createChildPop() {
  ...
  //Looks for best individual (obj[0])
    ...
    cout << ")" << endl;
		...
		// Input Variable: bestPmIn
		fl::scalar bestPmInValue = getBestPmInValue();
		bestPmIn->setInput(bestPmInValue);
    ...
    cout << "BEST_PM_IN=" << bestPmInValue << endl;
    cout << "PM_IN is " << pmIn->fuzzify(pmInValue) << endl;
    cout << "FIT is " << fit->fuzzify(fitValue) << endl;
    cout << "BEST_PM_IN is " << bestPmIn->fuzzify(bestPmInValue) << endl;
    ...
    // Stops the initialization stage
      ...
      // Transforms the value of pm from range [0, 1] to the range [minR, maxR]
...
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::fillExternalArchive(){
	for (int i = 0; i < offSpring->size(); i++){
		insertInArchive((*offSpring)[i]);
	}
}
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::multiObjectivize(vector <Individual*> *updatePop){
...
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::exchangePerformed(){
	multiObjectivize(population);	
}

long double NSGA2_MultiObjectivized_Fuzzy_B_Restart::getFitValue() {
...
long double NSGA2_MultiObjectivized_Fuzzy_B_Restart::getBestPmInValue() {
	long double bestImprovement = improvementHistory[improvementHistory.size() - 1];
	long double bestPmIn = pmHistory[pmHistory.size() - 1];
	int lowerbound = (improvementHistory.size() <= k)? 0 : improvementHistory.size() - k;

	for (int i = lowerbound; i < improvementHistory.size() - 1; i++) {
		if (improvementHistory[i] > bestImprovement) {
			bestImprovement = improvementHistory[i];
			bestPmIn = pmHistory[i];
		}
	}
...
Individual* NSGA2_MultiObjectivized_Fuzzy_B_Restart::lookForBestIndividual() {
...
double* NSGA2_MultiObjectivized_Fuzzy_B_Restart::getRestartInfo() {
  ...
  double *data = new double[3 + (1 + initStageValues.size()) + (1 + improvements.size() + numImpr) + (1 + memberships.size() + numMemb) +
                                (1 + weightedImprovements.size()) + (1 + pmHistory.size()) + (1 + improvementHistory.size())];
  int idx = 0;
  data[idx++] = pm;
...
void NSGA2_MultiObjectivized_Fuzzy_B_Restart::setRestartInfo(double *data) {
