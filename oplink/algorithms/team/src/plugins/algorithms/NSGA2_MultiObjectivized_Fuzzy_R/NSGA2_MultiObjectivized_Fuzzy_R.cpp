...
#include "NSGA2_MultiObjectivized_Fuzzy_R.h"
#include <float.h>


// Define una generación de búsqueda del algoritmo 
void NSGA2_MultiObjectivized_Fuzzy_R::runGeneration() {
...
bool NSGA2_MultiObjectivized_Fuzzy_R::init(const vector<string> &params) {
  ...
  rOut->setName("R_OUT");
	...
	rOut->addTerm(new fl::Triangle("NEG_GIANT", -0.54, -0.45, -0.36));
	rOut->addTerm(new fl::Triangle("NEG_HUGE", -0.45, -0.36, -0.27));
	rOut->addTerm(new fl::Triangle("NEG_HIGH", -0.36, -0.27, -0.18));
	rOut->addTerm(new fl::Triangle("NEG_MEDIUM", -0.27, -0.18, -0.09));
	rOut->addTerm(new fl::Triangle("NEG_LOW", -0.18, -0.09, 0));
	rOut->addTerm(new fl::Triangle("ZERO", -0.09, 0, 0.09));
	rOut->addTerm(new fl::Triangle("POS_LOW", 0, 0.09, 0.18));
	rOut->addTerm(new fl::Triangle("POS_MEDIUM", 0.09, 0.18, 0.27));
	rOut->addTerm(new fl::Triangle("POS_HIGH", 0.18, 0.27, 0.36));
	rOut->addTerm(new fl::Triangle("POS_HUGE", 0.27, 0.36, 0.45));
	rOut->addTerm(new fl::Triangle("POS_GIANT", 0.36, 0.45, 0.54));
	...
	rules_low->addRule(fl::FuzzyRule::parse("if R_IN is LOW and FIT is LOW and VAR is LOW then R_OUT is POS_LOW", fuzzyEngine));
	rules_low->addRule(fl::FuzzyRule::parse("if R_IN is LOW and FIT is LOW and VAR is MEDIUM then R_OUT is POS_LOW", fuzzyEngine));
	rules_low->addRule(fl::FuzzyRule::parse("if R_IN is LOW and FIT is LOW and VAR is HIGH then R_OUT is NEG_LOW", fuzzyEngine));
	...
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if R_IN is LOW_MED_B and FIT is LOW and VAR is LOW then R_OUT is POS_LOW", fuzzyEngine));
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if R_IN is LOW_MED_B and FIT is LOW and VAR is MEDIUM then R_OUT is POS_LOW", fuzzyEngine));
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if R_IN is LOW_MED_B and FIT is LOW and VAR is HIGH then R_OUT is NEG_LOW", fuzzyEngine));
	...
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if R_IN is LOW_MED_A and FIT is LOW and VAR is LOW then R_OUT is POS_LOW", fuzzyEngine));
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if R_IN is LOW_MED_A and FIT is LOW and VAR is MEDIUM then R_OUT is POS_LOW", fuzzyEngine));
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if R_IN is LOW_MED_A and FIT is LOW and VAR is HIGH then R_OUT is NEG_LOW", fuzzyEngine));
	...
	rules_medium->addRule(fl::FuzzyRule::parse("if R_IN is MEDIUM and FIT is LOW and VAR is LOW then R_OUT is POS_LOW", fuzzyEngine));
	rules_medium->addRule(fl::FuzzyRule::parse("if R_IN is MEDIUM and FIT is LOW and VAR is MEDIUM then R_OUT is POS_LOW", fuzzyEngine));
	rules_medium->addRule(fl::FuzzyRule::parse("if R_IN is MEDIUM and FIT is LOW and VAR is HIGH then R_OUT is NEG_LOW", fuzzyEngine));
	...
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if R_IN is MED_HIGH_A and FIT is LOW and VAR is LOW then R_OUT is POS_LOW", fuzzyEngine));
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if R_IN is MED_HIGH_A and FIT is LOW and VAR is MEDIUM then R_OUT is POS_LOW", fuzzyEngine));
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if R_IN is MED_HIGH_A and FIT is LOW and VAR is HIGH then R_OUT is NEG_LOW", fuzzyEngine));
	...
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if R_IN is MED_HIGH_B and FIT is LOW and VAR is LOW then R_OUT is POS_LOW", fuzzyEngine));
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if R_IN is MED_HIGH_B and FIT is LOW and VAR is MEDIUM then R_OUT is POS_LOW", fuzzyEngine));
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if R_IN is MED_HIGH_B and FIT is LOW and VAR is HIGH then R_OUT is NEG_LOW", fuzzyEngine));
	...
	rules_high->addRule(fl::FuzzyRule::parse("if R_IN is HIGH and FIT is LOW and VAR is LOW then R_OUT is POS_LOW", fuzzyEngine));
	rules_high->addRule(fl::FuzzyRule::parse("if R_IN is HIGH and FIT is LOW and VAR is MEDIUM then R_OUT is POS_LOW", fuzzyEngine));
	rules_high->addRule(fl::FuzzyRule::parse("if R_IN is HIGH and FIT is LOW and VAR is HIGH then R_OUT is NEG_LOW", fuzzyEngine));
...
void NSGA2_MultiObjectivized_Fuzzy_R::getSolution(MOFront *p) {
...
void NSGA2_MultiObjectivized_Fuzzy_R::printInfo(ostream &os) const {
	os << "Genetic Algorithm NSGA2_MultiObjectivized_Fuzzy_R" << endl;
 ...
 * ***********************************************************************/
void NSGA2_MultiObjectivized_Fuzzy_R::rankCrowdingPopulation() {
	int inserted = 0;
...
void NSGA2_MultiObjectivized_Fuzzy_R::rankPopulation() {
	vector < vector < Individual * > > fronts;
	vector < Individual *> notClassified;
 ...
 *  - Recombination
 * *****************************************************************************/
void NSGA2_MultiObjectivized_Fuzzy_R::createChildPop() {
...
void NSGA2_MultiObjectivized_Fuzzy_R::fillExternalArchive(){
	for (int i = 0; i < offSpring->size(); i++){
		insertInArchive((*offSpring)[i]);
	}
}
void NSGA2_MultiObjectivized_Fuzzy_R::multiObjectivize(vector <Individual*> *updatePop){
...
void NSGA2_MultiObjectivized_Fuzzy_R::exchangePerformed(){
	multiObjectivize(population);	
}

long double NSGA2_MultiObjectivized_Fuzzy_R::getFitValue() {
...
long double NSGA2_MultiObjectivized_Fuzzy_R::getVarValue() {
...
Individual* NSGA2_MultiObjectivized_Fuzzy_R::lookForBestIndividual() {
