#include "MultiObjNearestDistThreshold_Fuzzy_A_TKS.h"

#include <stdlib.h>
#include <math.h>
#include <float.h>

bool MultiObjNearestDistThreshold_Fuzzy_A_TKS::init(const vector<string> &params) {
  if (params.size() != 4){
  ...
  thrIn->setName("THR_IN");
	...
	multiObjFuzzyEngine->addInputVariable(var);
	
	// Output Variables
	thrOut = new fl::OutputVariable;
  thrOut->setName("THR_OUT");
	thrOut->setRange(-0.45, 0.45);
	thrOut->addTerm(new fl::Constant("NEG_GIANT", -0.45));
	thrOut->addTerm(new fl::Constant("NEG_HUGE", -0.36));
	thrOut->addTerm(new fl::Constant("NEG_HIGH", -0.27));
	thrOut->addTerm(new fl::Constant("NEG_MEDIUM", -0.18));
	thrOut->addTerm(new fl::Constant("NEG_LOW", -0.09));
	thrOut->addTerm(new fl::Constant("ZERO", 0));
	thrOut->addTerm(new fl::Constant("POS_LOW", 0.09));
	thrOut->addTerm(new fl::Constant("POS_MEDIUM", 0.18));
	thrOut->addTerm(new fl::Constant("POS_HIGH", 0.27));
	thrOut->addTerm(new fl::Constant("POS_HUGE", 0.36));
	thrOut->addTerm(new fl::Constant("POS_GIANT", 0.45));
	...
	rules_low->addRule(fl::FuzzyRule::parse("if THR_IN is LOW and FIT is LOW and VAR is LOW then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_low->addRule(fl::FuzzyRule::parse("if THR_IN is LOW and FIT is LOW and VAR is MEDIUM then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_low->addRule(fl::FuzzyRule::parse("if THR_IN is LOW and FIT is LOW and VAR is HIGH then THR_OUT is POS_LOW", multiObjFuzzyEngine));
	...
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if THR_IN is LOW_MED_B and FIT is LOW and VAR is LOW then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if THR_IN is LOW_MED_B and FIT is LOW and VAR is MEDIUM then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_low_med_b->addRule(fl::FuzzyRule::parse("if THR_IN is LOW_MED_B and FIT is LOW and VAR is HIGH then THR_OUT is POS_LOW", multiObjFuzzyEngine));
	...
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if THR_IN is LOW_MED_A and FIT is LOW and VAR is LOW then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if THR_IN is LOW_MED_A and FIT is LOW and VAR is MEDIUM then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_low_med_a->addRule(fl::FuzzyRule::parse("if THR_IN is LOW_MED_A and FIT is LOW and VAR is HIGH then THR_OUT is POS_LOW", multiObjFuzzyEngine));
	...
	rules_medium->addRule(fl::FuzzyRule::parse("if THR_IN is MEDIUM and FIT is LOW and VAR is LOW then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_medium->addRule(fl::FuzzyRule::parse("if THR_IN is MEDIUM and FIT is LOW and VAR is MEDIUM then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_medium->addRule(fl::FuzzyRule::parse("if THR_IN is MEDIUM and FIT is LOW and VAR is HIGH then THR_OUT is POS_LOW", multiObjFuzzyEngine));
	...
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if THR_IN is MED_HIGH_A and FIT is LOW and VAR is LOW then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if THR_IN is MED_HIGH_A and FIT is LOW and VAR is MEDIUM then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_med_high_a->addRule(fl::FuzzyRule::parse("if THR_IN is MED_HIGH_A and FIT is LOW and VAR is HIGH then THR_OUT is POS_LOW", multiObjFuzzyEngine));
	...
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if THR_IN is MED_HIGH_B and FIT is LOW and VAR is LOW then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if THR_IN is MED_HIGH_B and FIT is LOW and VAR is MEDIUM then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_med_high_b->addRule(fl::FuzzyRule::parse("if THR_IN is MED_HIGH_B and FIT is LOW and VAR is HIGH then THR_OUT is POS_LOW", multiObjFuzzyEngine));
	...
	rules_high->addRule(fl::FuzzyRule::parse("if THR_IN is HIGH and FIT is LOW and VAR is LOW then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_high->addRule(fl::FuzzyRule::parse("if THR_IN is HIGH and FIT is LOW and VAR is MEDIUM then THR_OUT is NEG_LOW", multiObjFuzzyEngine));
	rules_high->addRule(fl::FuzzyRule::parse("if THR_IN is HIGH and FIT is LOW and VAR is HIGH then THR_OUT is POS_LOW", multiObjFuzzyEngine));
  ...
  multiObjFuzzyEngine->configure("Minimum", "Maximum", "Minimum", "", "WeightedAverage");
...
void MultiObjNearestDistThreshold_Fuzzy_A_TKS::convertToMultiObjective(vector<Individual *> *population, vector<Individual *> *offSpring, vector<Individual *> *reference, int ordObj, int numberGeneration) {
...
Individual* MultiObjNearestDistThreshold_Fuzzy_A_TKS::lookForBestIndividual(vector<Individual *> *population, vector<Individual *> *offSpring) {
...
long double MultiObjNearestDistThreshold_Fuzzy_A_TKS::getFitValue() {
...
long double MultiObjNearestDistThreshold_Fuzzy_A_TKS::getVarValue() {
...
bool MultiObjNearestDistThreshold_Fuzzy_A_TKS::updateObjectiveValue(vector<Individual *> *population, vector<Individual *> *offSpring, Individual *best, int ordObj) {
