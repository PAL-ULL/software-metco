...
bool AFP_R_AuxData::init (const vector<string> &params){
	//Check parameters
	if (params.size() < 3){
  ...
  // Uses the parameter maxMutated as auxiliar data belonging to the individual
  this->setAuxDataSize(1);
...
void AFP_R_AuxData::dependentMutation(double pm){
	switch (mutationType){
		case MUTATION_UNIFORM:
			mutationUniform(pm);
			break;
...
void AFP_R_AuxData::dependentCrossover(Individual *ind) {
	int trxRandom;
		...
		case CROSSOVER_INTERFERED:
  		trxRandom = (int) ((double)(ind->getNumberOfVar()) * rand() / (RAND_MAX + 1.0));
	 		for (int i = 0; i < ind->getNumberOfVar(); i++) {
      		...
      		((AFP_R_AuxData*)ind)->removeAssignTRX(i, tmp);
    		}
  		}
			break;
		case CROSSOVER_UNIFORM:
			for (unsigned int i = 0; i < getNumberOfVar(); i++){
				if (rand() > (RAND_MAX / 2)){
					...
					((AFP_R_AuxData*)ind)->removeAssignTRX(i, getVar(i));
...
void AFP_R_AuxData::mutationUniform(double pm){
...
void AFP_R_AuxData::mutationRename(double pm){
...
// At the end, performs a localSearch
void AFP_R_AuxData::mutationNeighbours(double pm){
	MiSetInt *newSectorsToTry = new MiSetInt(problem->getHighestSector());
	vector<int> previouslyInfluencedSectors;
	set<int> interfPrevious;

  // ****** NEW ******
  maxMutated = this->getAuxData(0);

  cout << "pm at NM operator: " << pm << endl << flush;
  cout << "maxMutated at NM operator: " << maxMutated << endl << flush;
...
void AFP_R_AuxData::calculateInfluencedTrxs(){
...
void AFP_R_AuxData::removeAssignTRX(int trx, int freq){
	int dif;
...
void AFP_R_AuxData::removeAssignTRX2(int trx, int freq, set<int> &interfPrevious){
	int dif;
...
void AFP_R_AuxData::randomAssignment(){
	for (unsigned int i = 0; i < (*trxs).size(); i++){
		vector<int> *trxFreq = (*trxs)[i].validFrequencies;
...
Individual *AFP_R_AuxData::clone() const{ 
	AFP_R_AuxData *newInd = new AFP_R_AuxData();
	//copy the current costs
	newInd->cacheData = cacheData;
  //copy aux data
	newInd->setAuxDataSize(getAuxDataSize());
  for (int i = 0; i < getAuxDataSize(); i++)
    newInd->setAuxData(i, getAuxData(i));
...
void AFP_R_AuxData::deserialized(){
	//Calculamos la tabla cache
	setCacheDataSize(problem->getIndexesInTable());
  setAuxDataSize(1);
	problem->completeEvaluation(this);
...
*/
void AFP_R_AuxData::localSearch(){
...
void AFP_R_AuxData::improveSectors(MiSetInt *newSectorsToTry){
...
bool AFP_R_AuxData::improveSector(int selectedSector){
...
bool AFP_R_AuxData::assignTrxToFrequency(set<int> &notAssignedTrxs, int freq, multiset<int> &possibleFrequencies, vector<int> &assignedTrxs){
...
void AFP_R_AuxData::assignTRX(int trx, int freq){
	int dif;
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		if ((int)getVar(iter->first) == freq){
			setObj(0, getObj(0) + (*coCostBoth)[trx][iter->first]);
...
void AFP_R_AuxData::removeTrx(int trx) {
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		setObj(0, getObj(0) - getCacheData(iter->second));
		setCacheData(iter->second, 0);
	}
...
void AFP_R_AuxData::getAssignmentCost(int trx, vector<double> &assignmentCosts) const{
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
...
void AFP_R_AuxData::print(ostream &os) const {
...
void AFP_R_AuxData::addVictimInterfererCostValue(int index, double cost){
	setCacheData(index, getCacheData(index) + cost);
}

// Print intialization error
void AFP_R_AuxData::initialization_error(){
	cout << "Incorrect AFP_R_AuxData initalization: instance_file crossoverType mutationType mutationParams" << endl;
	...
	cout << "  " << MUTATION_NEIGHBOURS << ": Mutation_Neighbours - No parameters" << endl;
}
...
// value
void AFP_R_AuxData::evaluate(){
}

//Static members of AFP class
AFPProblem *AFP_R_AuxData::problem = NULL;
vector<PairSectorCost> *AFP_R_AuxData::sortCosts = NULL;
vector<int> *AFP_R_AuxData::dependentSectors;
vector<int> *AFP_R_AuxData::independentSectors;
vector< vector<int> > *AFP_R_AuxData::sectors2trxs;
vector<TRX> *AFP_R_AuxData::trxs;
vector< vector <double> > *AFP_R_AuxData::adjCostBoth;
vector< vector <double> > *AFP_R_AuxData::coCostBoth;
vector<map <int, int> > *AFP_R_AuxData::victimOrInterfererTrxCost;
vector< set <int> > *AFP_R_AuxData::sectorsInterfereOrVictim;
vector< set <int> > AFP_R_AuxData::directInfluencedTrxs;
bool AFP_R_AuxData::origCodeLoaded = false;
int AFP_R_AuxData::firstInitialization = 1;
...
void AFPProblem::completeEvaluation(AFP_R_AuxData *ind){
	int i;
	double cost = 0;
