...
bool AFP_SelfAdaptive::init (const vector<string> &params){
	//Check parameters
	if (params.size() < 3){
	...
	setNumberOfVar(trxs->size() + 1);
	setNumberOfObj(1);
	return true;
}
		
double AFP_SelfAdaptive::getMaximum (const int i) const {
	if (i == (getNumberOfVar() - 1))
		return 1;
	return (*(*trxs)[i].validFrequencies)[(*trxs)[i].validFrequencies->size() - 1];
}

double AFP_SelfAdaptive::getMinimum (const int i) const {
	if (i == (getNumberOfVar() - 1))
		return 0;
	return (*(*trxs)[i].validFrequencies)[0];
}
...
void AFP_SelfAdaptive::dependentMutation(double pm){
	switch (mutationType){
		case MUTATION_UNIFORM:
			mutationUniform(pm);
			break;
...
void AFP_SelfAdaptive::dependentCrossover(Individual *ind) {
	int trxRandom;
		...
		case CROSSOVER_INTERFERED:
	 		for (int i = 0; i < ind->getNumberOfVar(); i++) {
				if (i == (getNumberOfVar() - 1)) {
  				...
  				trxRandom = (int) ((double)(ind->getNumberOfVar() - 1) * rand() / (RAND_MAX + 1.0));
 	  			if (((*coCostBoth)[trxRandom][i] > 0) || (trxRandom == i)){
      			double tmp = getVar(i);
      			removeAssignTRX(i, ind->getVar(i));
      			((AFP_SelfAdaptive*)ind)->removeAssignTRX(i, tmp);
    			}
  			}
			}
			break;
			...
			for (unsigned int i = 0; i < getNumberOfVar(); i++){
				if (rand() > (RAND_MAX / 2)){
					if (i == (getNumberOfVar() - 1)) {
						...
						double tmp = ind->getVar(i);
						((AFP_SelfAdaptive*)ind)->removeAssignTRX(i, getVar(i));
						//ind->setVar(i, getVar(i));
...
void AFP_SelfAdaptive::mutationUniform(double pm){
...
void AFP_SelfAdaptive::mutationRename(double pm){
...
// At the end, performs a localSearch
void AFP_SelfAdaptive::mutationNeighbours(double pm){
	MiSetInt *newSectorsToTry = new MiSetInt(problem->getHighestSector());
	...
	// Mutates the variable which represents the threshold value
	float vmut = ((double) rand()) / (double) (RAND_MAX);
	if (vmut < pm) {
		double tmp = ((double) rand () / (double) RAND_MAX);
		int i = getNumberOfVar() - 1;
		setVar(i, tmp * (getMaximum(i) - getMinimum(i)) + getMinimum(i));
	}
}
...
void AFP_SelfAdaptive::calculateInfluencedTrxs(){
...
void AFP_SelfAdaptive::removeAssignTRX(int trx, int freq){
	int dif;
...
void AFP_SelfAdaptive::removeAssignTRX2(int trx, int freq, set<int> &interfPrevious){
	int dif;
...
void AFP_SelfAdaptive::randomAssignment(){
	for (unsigned int i = 0; i < (*trxs).size(); i++){
		vector<int> *trxFreq = (*trxs)[i].validFrequencies;
...
Individual *AFP_SelfAdaptive::clone() const{ 
	AFP_SelfAdaptive *newInd = new AFP_SelfAdaptive();
...
void AFP_SelfAdaptive::cloned(){
	//Calculamos la tabla cache
	setCacheDataSize(problem->getIndexesInTable());
	problem->completeEvaluation(this);
}

void AFP_SelfAdaptive::deserialized(){
	//Calculamos la tabla cache
	setCacheDataSize(problem->getIndexesInTable());
	problem->completeEvaluation(this);
}
...
*/
void AFP_SelfAdaptive::localSearch(){
...
void AFP_SelfAdaptive::improveSectors(MiSetInt *newSectorsToTry){
...
bool AFP_SelfAdaptive::improveSector(int selectedSector){
...
bool AFP_SelfAdaptive::assignTrxToFrequency(set<int> &notAssignedTrxs, int freq, multiset<int> &possibleFrequencies, vector<int> &assignedTrxs){
...
void AFP_SelfAdaptive::assignTRX(int trx, int freq){
	int dif;
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		if ((int)getVar(iter->first) == freq){
			setObj(0, getObj(0) + (*coCostBoth)[trx][iter->first]);
...
void AFP_SelfAdaptive::removeTrx(int trx) {
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		setObj(0, getObj(0) - getCacheData(iter->second));
		setCacheData(iter->second, 0);
	}
...
void AFP_SelfAdaptive::getAssignmentCost(int trx, vector<double> &assignmentCosts) const{
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
...
void AFP_SelfAdaptive::print(ostream &os) const {
...
void AFP_SelfAdaptive::addVictimInterfererCostValue(int index, double cost){
	setCacheData(index, getCacheData(index) + cost);
}

// Print intialization error
void AFP_SelfAdaptive::initialization_error(){
	cout << "Incorrect AFP_SelfAdaptive initalization: instance_file crossoverType mutationType mutationParams" << endl;
...
// value
void AFP_SelfAdaptive::evaluate(){
}

//Static members of AFP class
AFPProblem *AFP_SelfAdaptive::problem = NULL;
vector<PairSectorCost> *AFP_SelfAdaptive::sortCosts = NULL;
vector<int> *AFP_SelfAdaptive::dependentSectors;
vector<int> *AFP_SelfAdaptive::independentSectors;
vector< vector<int> > *AFP_SelfAdaptive::sectors2trxs;
vector<TRX> *AFP_SelfAdaptive::trxs;
vector< vector <double> > *AFP_SelfAdaptive::adjCostBoth;
vector< vector <double> > *AFP_SelfAdaptive::coCostBoth;
vector<map <int, int> > *AFP_SelfAdaptive::victimOrInterfererTrxCost;
vector< set <int> > *AFP_SelfAdaptive::sectorsInterfereOrVictim;
vector< set <int> > AFP_SelfAdaptive::directInfluencedTrxs;
bool AFP_SelfAdaptive::origCodeLoaded = false;
int AFP_SelfAdaptive::firstInitialization = 1;
...
void AFPProblem::completeEvaluation(AFP_SelfAdaptive *ind){
	int i;
	double cost = 0;
