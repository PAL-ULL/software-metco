...
// Problema a resolver
ProblemRND *RND::problem = NULL;	
map< int, vector< vector <int> > > RND::bsRegion;
vector< vector< vector< vector <int> > > > RND::RegionBs;
vector< vector<int> > RND::posiciones;
string RND::instanceFile;
unsigned int RND::type_instance;
vector<string> RND::crossParamNames;
vector<string> RND::crossParamValues;
...
bool RND::init(const vector<string> &params) {
	// Check for the problem parameters
	if (params.size() < 1){
		cout << "Error problem RND: type_fich other_parameters" << endl;
	}
		
	int currentArg = 0;
	if (atoi(params[0].c_str()) == 1){
		if (params.size() < 5) {
			cout << "Error problem RND: type_fich fich_dim fich_inst radio type_crossover Other_parameters" << endl;
			return false;
		}
		currentArg = 4;
	} else if (atoi(params[0].c_str()) == 2){
		if (params.size() < 3){
			cout << "Error problem RND: type_fich fich_inst type_crossover Other_parameters" << endl;
			return false;
		}
		currentArg = 2; 
	}
	...
	if ((type_crossover == CROSSOVER_OPC) && (params.size() != currentArg + 1)){//Crossover OPC
		cout << "Error problem RND with crossover OPC: type_fich fich_dim fich_inst radio type_crossover" << endl;
		return false;
	} else if ((type_crossover == CROSSOVER_GEOGRAPHIC_CIRCULAR) && (params.size() != currentArg + 2)){//Crossover geographic Circular
		cout << "Error problem RND with crossover Geographic Circular: type_fich fich_dim fich_inst radio type_crossover radio" << endl;
		return false;
	} else if ((type_crossover == CROSSOVER_GEOGRAPHIC_RECTANGULAR) && (params.size() != currentArg + 4)){
		cout << "Error problem RND with crossover Geographic Rectangular: type_fich fich_dim fich_inst radio type_crossover type_file Min_dim Max_dim" << endl;
		return false;
	} else if ((type_crossover == CROSSOVER_HUX) && (params.size() != currentArg + 1)){//Crossover HUX
		cout << "Error problem RND with crossover HUX: type_fich fich_dim fich_inst radio type_crossover" << endl;
		return false;
	} 
	if ((type_crossover != CROSSOVER_OPC) && (type_crossover != CROSSOVER_GEOGRAPHIC_CIRCULAR) && (type_crossover != CROSSOVER_GEOGRAPHIC_RECTANGULAR) && (type_crossover != CROSSOVER_HUX)){
		cout << "Error problem RND with unknown crossover id" << endl;
		return false;
	}
  ...
  if (problem == NULL){//only initialize once the problem (and the info about the parameters)
		...
		if (atoi(params[0].c_str()) == 1){
			...
			cout << "Error problem RND: incorrect type_fich" << endl;
		}
	}
...
unsigned int RND::getOptDirection(const int i) const{ 
...
unsigned int RND::getNeighbourhoodSize() const {
...
Individual *RND::getNeighbour(const int i) const {

	// Crear un nuevo individuo para trabajar sobre el
	RND *newind = (RND*)internalClone();
...
void RND::getNeighbourObjectives(const int i, vector<double> &objectives) const {
...
void RND::removeBs(const unsigned int bsId) {
...
void RND::addBs(const unsigned int bsId) {
...
void RND::dependentMutation(double pm) {
...
void RND::crossover_opc(Individual* ind) {
	int nvars = getNumberOfVar();
...
//
void RND::crossover_geographic_circular(Individual* ind) {
	int nvars = getNumberOfVar();
	...
	for (unsigned int i = 0; i < bsRegion[radio_region][bs_geographic].size(); i++) {
		...
		if ((tmp2 == 1) && (tmp1 == 0)){
			((RND*)(ind))->addBs(bsRegion[radio_region][bs_geographic][i]);
			removeBs(bsRegion[radio_region][bs_geographic][i]);
		} else if ((tmp2 == 0) && (tmp1 == 1)){
			((RND*)(ind))->removeBs(bsRegion[radio_region][bs_geographic][i]);
			addBs(bsRegion[radio_region][bs_geographic][i]);
		}
	}
}

void RND::crossover_geographic_rectangular(Individual* ind){
	...
	for (unsigned int i = 0; i < antenas.size(); i++){
		...
		if ((tmp2 == 1) && (tmp1 == 0)){
			((RND*)(ind))->addBs(antenas[i]);
			removeBs(antenas[i]);
		} else if ((tmp2 == 0) && (tmp1 == 1)){
			((RND*)(ind))->removeBs(antenas[i]);
			addBs(antenas[i]);
		}
	}
...
void RND::crossover_hux(Individual* ind) {
...
unsigned int ProblemRND::calculateCoverage(RND *rndInd, unsigned int &usedBs, vector< vector<int> > *grid) {
...
//Info de la mutacion
string RND::getMutationName() const{
	return "BitFlipMutation";
}

//Info del crossover
string RND::getCrossoverName() const{
	if (type_crossover == 0){
...
int RND::getCrossoverNumberOfParameters() const{
	return crossParamNames.size();
}

string RND::getCrossoverParameterName(int i) const{
	return crossParamNames[i];
}

string RND::getCrossoverParameterValue(int i) const{
	return crossParamValues[i];
}

//Info de los objetivos
string RND::getObjName(int i) const{
	if (i == 0)
		return "numberBS";
	if (i == 1)
		return "coverRate";
