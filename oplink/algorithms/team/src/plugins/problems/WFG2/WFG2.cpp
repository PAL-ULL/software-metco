...
bool WFG2::init (const vector<string> &params) {
	// Comprueba los parametros del problema
	if (params.size() != 1) {
		cout << "Error WFG2: Incorrect parameters (number_of_objectives)" << endl;
		return false;
	}
...
void WFG2::evaluate (void) {
  ...
  vector<double> y;
	...
	y = WFG2_t1(y, k);
	y = WFG2_t2(y, k);
	y = WFG2_t3(y, k, getNumberOfObj());

	// Calcula los valores de las funciones objetivo
	vector<double> result = WFG2_shape(y);
