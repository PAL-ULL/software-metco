...
#include "TwoDPack_MultiObjectivized_Aggregation.h"
#include <limits.h>
#include <set>
#include <iostream>
#include <fstream>
...
TwoDPack_MultiObjectivized_Aggregation::~TwoDPack_MultiObjectivized_Aggregation(){
	nInds--;
	if (nInds == 0){
		endLocalSearch1();
		endLocalSearch2();
...
void TwoDPack_MultiObjectivized_Aggregation::initLocalSearch1(){
...
void TwoDPack_MultiObjectivized_Aggregation::endLocalSearch1(){
	for (unsigned int i = 0; i < neighboursToExplore->size(); i++)
		delete (*neighboursToExplore)[i];
	neighboursToExplore = NULL;
}

void TwoDPack_MultiObjectivized_Aggregation::initLocalSearch2(){
...
void TwoDPack_MultiObjectivized_Aggregation::endLocalSearch2(){
	for (unsigned int i = 0; i < partnersToExplore->size(); i++)
		delete (*partnersToExplore)[i];
	partnersToExplore = NULL;
}

bool TwoDPack_MultiObjectivized_Aggregation::init (const vector<string> &params){
	if (params.size() != 7){
	...
	//grid.insert(grid.begin(), twoDPackingProblem->getXSize(), vector<int>(twoDPackingProblem->getYSize(), -1));
...
void TwoDPack_MultiObjectivized_Aggregation::evaluate (void){
	calculateFitness();
}

//Generación de aleatorio y evaluación de objetivo
void TwoDPack_MultiObjectivized_Aggregation::restart(void){
	generateRandom();
	calculateFitness();
	localSearch();
}
...
void TwoDPack_MultiObjectivized_Aggregation::generateRandom(){
	for (int i = 0; i < twoDPackingProblem->getXSize(); i++){
		for (int j = 0; j < twoDPackingProblem->getYSize(); j++){
...
void TwoDPack_MultiObjectivized_Aggregation::update(const int x1, const int y1, const int x2, const int y2){
	int n1 = getGrid(x1, y1);
	int n2 = getGrid(x2, y2);
	appearance[n1][n2]++;
	if (appearance[n1][n2] == 1) {
    if (twoDPackingProblem->getValue(n1, n2) != 0) {
      setObj(0, getObj(0) + twoDPackingProblem->getValue(n1, n2));
      if (twoDPackingProblem->getPair2Obj(n1, n2) == 1)
        setObj(twoDPackingProblem->getPair2Obj(n1, n2), getObj(twoDPackingProblem->getPair2Obj(n1, n2)) + twoDPackingProblem->getValue(n1, n2));
    }
  }
	if (n1 != n2)
		appearance[n2][n1]++;
...
void TwoDPack_MultiObjectivized_Aggregation::calculateFitness(){
	setObj(0, 0);
	setObj(1, 0);
...
// Actualizar el fitness y aparición de parejas considerando la casilla (x1, y1) - (x2, y2)
void TwoDPack_MultiObjectivized_Aggregation::updatePartial(const int x1, const int y1, const int x2, const int y2) {
	int n1 = getGrid(x1, y1);
	int n2 = getGrid(x2, y2);
	pAppearance[n1][n2]++;
...
void TwoDPack_MultiObjectivized_Aggregation::calculatePartialFitness(const int initX, const int endX, const int initY, const int endY) {
	pFitness = 0;
	for (int i = 0; i < twoDPackingProblem->getMaxNumber(); i++) {
		for (int j = 0; j < twoDPackingProblem->getMaxNumber(); j++) {
			pAppearance[i][j] = 0;
...
//V31 ....
//...
ostream& operator<< (ostream &os, const TwoDPack_MultiObjectivized_Aggregation &twoDPackingIndividual){
...
void TwoDPack_MultiObjectivized_Aggregation::localSearch(){
...
void TwoDPack_MultiObjectivized_Aggregation::localSearch1(){
	bool improved = false;
...
//- Por eficiencia cuando se cambia a un vecino, no se vuelve a rellenar la lista de vecinos a los
//que se puede mover, sino que sigue con los que quedan.
void TwoDPack_MultiObjectivized_Aggregation::localSearch2(){
	bool improved = false;
...
int TwoDPack_MultiObjectivized_Aggregation::removeAssignation(const int x, const int y){
	int removeFitness = 0;
	...
	for (int i = initX; i <= finX; i++){
        ...
        if (twoDPackingProblem->getValue(value, value2) != 0) {
				  removeFitness += twoDPackingProblem->getValue(value, value2);
          setObj(0, getObj(0) - twoDPackingProblem->getValue(value, value2));
          if (twoDPackingProblem->getPair2Obj(value, value2) == 1)
            setObj(twoDPackingProblem->getPair2Obj(value, value2), getObj(twoDPackingProblem->getPair2Obj(value, value2)) - twoDPackingProblem->getValue(value, value2));
        }
			}
		}
	}
...
void TwoDPack_MultiObjectivized_Aggregation::getAssignationCosts(const int x, const int y, vector<int> &costValues){
...
// Obtener el fitness asociado a realizar una asignación
int TwoDPack_MultiObjectivized_Aggregation::getAssignationCost(const int x, const int y, const int value){
...
int TwoDPack_MultiObjectivized_Aggregation::getAssignationCost(const int x1, const int y1, const int v1, const int x2, const int y2, const int v2){
	set <pair <int, int> > newPartners;
	int fitnessAssignation = 0;
	int x, y, value;
	for (int i = 0; i < 2; i++){
...
// Obtener el coste asociado a hacer 2 asignaciones de casillas contiguas
int TwoDPack_MultiObjectivized_Aggregation::getAssignationCost(const int x1, const int y1, const int v1, const int x2, const int y2, const int v2, const int x3, const int y3, const int v3){
	set <pair <int, int> > newPartners;
	int fitnessAssignation = 0;
	int x, y, value;
	for (int i = 0; i < 3; i++){
...
void TwoDPack_MultiObjectivized_Aggregation::makeAssignation(const int x, const int y, const int value){
	int initX = x>0?x-1:0;
	int initY = y>0?y-1:0;
	int finX = ((x+1)<twoDPackingProblem->getXSize())?x+1:x;
	int finY = ((y+1)<twoDPackingProblem->getYSize())?y+1:y;
...
bool TwoDPack_MultiObjectivized_Aggregation::improvePosition(const int x, const int y){
...
bool TwoDPack_MultiObjectivized_Aggregation::improvePosition(const Position &p1, const Position &p2, const Position &p3){
	vector<int> explored(twoDPackingProblem->getMaxNumber(), 0);
	...
	if (getObj(0) != (originalFitness - valuep1p2p3 + actualValue)){
		cout << "Fail 1: " << getObj(0) << " " << originalFitness - valuep1p2p3 + actualValue << endl;
		exit(-1);
	}
	return (actualValue > valuep1p2p3);
}
// Busca la mejor asignación posible para las casillas p1 y p2 (a la vez)
bool TwoDPack_MultiObjectivized_Aggregation::improvePosition(const Position &p1, const Position &p2){
	vector<int> explored(twoDPackingProblem->getMaxNumber(), 0);
	...
	if ((int)getObj(0) != (originalFitness - valuep1p2 + actualValue)){
		cout << "Fail 2: " << getObj(0) << " " << originalFitness - valuep1p2 + actualValue << endl;
		exit(-1);
	}
...
void TwoDPack_MultiObjectivized_Aggregation::mutationReorder(){
...
void TwoDPack_MultiObjectivized_Aggregation::mutationUniform(){
...
// Mutación uniforme: se elige un valor de probabilidad de mutacion mutProb entre mutProbLow y mutProbHigh
// cada elemento es mutado con una probabilidad mutProb
void TwoDPack_MultiObjectivized_Aggregation::mutationUniformValid(){
	double mutProb = mutProbLow + (mutProbHigh - mutProbLow) * ((double)(rand()) / RAND_MAX);
	for (int i = 0; i < twoDPackingProblem->getXSize(); i++){
		for (int j = 0; j < twoDPackingProblem->getYSize(); j++){
			if((1.0*rand()/(RAND_MAX+1.0)) < mutProb){
...
void TwoDPack_MultiObjectivized_Aggregation::mutationWindow() {
...
void TwoDPack_MultiObjectivized_Aggregation::dependentMutation( double pm ) {
	switch(typeMutation){
		case 0:
			mutationUniformValid();
			break;
...
void TwoDPack_MultiObjectivized_Aggregation::subStringCrossover(TwoDPack_MultiObjectivized_Aggregation *ind){
	int columna = (int)((twoDPackingProblem->getXSize()) * ((double)(rand()/(RAND_MAX + 1.0))));
...
void TwoDPack_MultiObjectivized_Aggregation::dependentCrossover (Individual *ind){
	subStringCrossover((TwoDPack_MultiObjectivized_Aggregation *) ind);	
...
Individual* TwoDPack_MultiObjectivized_Aggregation::clone() const{
	TwoDPack_MultiObjectivized_Aggregation *newInd = new TwoDPack_MultiObjectivized_Aggregation();
	//newInd->grid = grid;
...
const TwoDPackingProblem* TwoDPack_MultiObjectivized_Aggregation::twoDPackingProblem = NULL;
vector< pair<int, int> * >* TwoDPack_MultiObjectivized_Aggregation::neighboursToExplore = NULL;
vector< pair<Position, Position> * >* TwoDPack_MultiObjectivized_Aggregation::partnersToExplore = NULL;
int TwoDPack_MultiObjectivized_Aggregation::nInds = 0;
...
TwoDPackingProblem::TwoDPackingProblem(const string &fileName){
  ...
  // Sort pairs by its value
  PairValueStruct tmp;

  for (int i = 0; i < partners.size(); i++) {
    for (int j = 0; j < partners[i].size(); j++) {
      if (partners[i][j] >= i) {
        tmp.pairA = i;
        tmp.pairB = partners[i][j];
        tmp.value = partnersValues[i][partners[i][j]];
        orderedPairs.push_back(tmp);
      }
    }
  }

  sort(orderedPairs.begin(), orderedPairs.end(), cmpPairValueStruct);

  pair2Obj.insert(pair2Obj.begin(), maxNumber, vector<int>(maxNumber, -20));

  int counter = 0;

  for (int i = 0; i < orderedPairs.size(); i++) {
    pair2Obj[orderedPairs[i].pairA][orderedPairs[i].pairB] = counter % 2;
    pair2Obj[orderedPairs[i].pairB][orderedPairs[i].pairA] = counter % 2;
    counter++;
  }
}
