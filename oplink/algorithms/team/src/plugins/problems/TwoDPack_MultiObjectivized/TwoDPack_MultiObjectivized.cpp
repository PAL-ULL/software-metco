...
#include "TwoDPack_MultiObjectivized.h"
#include <limits.h>
#include <set>
#include <iostream>
#include <fstream>
...
TwoDPack_MultiObjectivized::~TwoDPack_MultiObjectivized(){
	nInds--;
	if (nInds == 0){
		endLocalSearch1();
		endLocalSearch2();
...
void TwoDPack_MultiObjectivized::initLocalSearch1(){
...
void TwoDPack_MultiObjectivized::endLocalSearch2(){
	for (unsigned int i = 0; i < partnersToExplore->size(); i++)
		delete (*partnersToExplore)[i];
	partnersToExplore = NULL;
}

bool TwoDPack_MultiObjectivized::init (const vector<string> &params){
	if (params.size() != 7){
...
void TwoDPack_MultiObjectivized::evaluate (void){
	calculateFitness();
}

//Generación de aleatorio y evaluación de objetivo
void TwoDPack_MultiObjectivized::restart(void){
	generateRandom();
	calculateFitness();
	localSearch();
}
...
void TwoDPack_MultiObjectivized::generateRandom(){
	for (int i = 0; i < twoDPackingProblem->getXSize(); i++){
		for (int j = 0; j < twoDPackingProblem->getYSize(); j++){
...
void TwoDPack_MultiObjectivized::update(const int x1, const int y1, const int x2, const int y2){
...
void TwoDPack_MultiObjectivized::calculateFitness(){
	setObj(0, 0);
	setObj(1, 0);
...
// Actualizar el fitness y aparición de parejas considerando la casilla (x1, y1) - (x2, y2)
void TwoDPack_MultiObjectivized::updatePartial(const int x1, const int y1, const int x2, const int y2) {
	int n1 = getGrid(x1, y1);
	int n2 = getGrid(x2, y2);
	pAppearance[n1][n2]++;
...
void TwoDPack_MultiObjectivized::calculatePartialFitness(const int initX, const int endX, const int initY, const int endY) {
	pFitness = 0;
	for (int i = 0; i < twoDPackingProblem->getMaxNumber(); i++) {
		for (int j = 0; j < twoDPackingProblem->getMaxNumber(); j++) {
			pAppearance[i][j] = 0;
...
//V31 ....
//...
ostream& operator<< (ostream &os, const TwoDPack_MultiObjectivized &twoDPackingIndividual){
...
void TwoDPack_MultiObjectivized::localSearch(){
...
void TwoDPack_MultiObjectivized::localSearch1(){
	bool improved = false;
...
//- Por eficiencia cuando se cambia a un vecino, no se vuelve a rellenar la lista de vecinos a los
//que se puede mover, sino que sigue con los que quedan.
void TwoDPack_MultiObjectivized::localSearch2(){
	bool improved = false;
...
int TwoDPack_MultiObjectivized::removeAssignation(const int x, const int y){
	int removeFitness = 0;
...
void TwoDPack_MultiObjectivized::getAssignationCosts(const int x, const int y, vector<int> &costValues){
...
// Obtener el fitness asociado a realizar una asignación
int TwoDPack_MultiObjectivized::getAssignationCost(const int x, const int y, const int value){
...
int TwoDPack_MultiObjectivized::getAssignationCost(const int x1, const int y1, const int v1, const int x2, const int y2, const int v2){
	set <pair <int, int> > newPartners;
	int fitnessAssignation = 0;
	int x, y, value;
	for (int i = 0; i < 2; i++){
...
// Obtener el coste asociado a hacer 2 asignaciones de casillas contiguas
int TwoDPack_MultiObjectivized::getAssignationCost(const int x1, const int y1, const int v1, const int x2, const int y2, const int v2, const int x3, const int y3, const int v3){
	set <pair <int, int> > newPartners;
	int fitnessAssignation = 0;
	int x, y, value;
	for (int i = 0; i < 3; i++){
...
void TwoDPack_MultiObjectivized::makeAssignation(const int x, const int y, const int value){
	int initX = x>0?x-1:0;
	int initY = y>0?y-1:0;
	int finX = ((x+1)<twoDPackingProblem->getXSize())?x+1:x;
	int finY = ((y+1)<twoDPackingProblem->getYSize())?y+1:y;
...
bool TwoDPack_MultiObjectivized::improvePosition(const int x, const int y){
...
bool TwoDPack_MultiObjectivized::improvePosition(const Position &p1, const Position &p2, const Position &p3){
	vector<int> explored(twoDPackingProblem->getMaxNumber(), 0);
...
bool TwoDPack_MultiObjectivized::improvePosition(const Position &p1, const Position &p2){
	vector<int> explored(twoDPackingProblem->getMaxNumber(), 0);
	vector<AssignationCost *> p1costs, p2costs;
	int originalFitness = (int)getObj(0) + (int)getObj(1);
	...
	if ((int)getObj(0) + (int)getObj(1) != (originalFitness - valuep1p2 + actualValue)){
...
void TwoDPack_MultiObjectivized::mutationReorder(){
...
void TwoDPack_MultiObjectivized::mutationUniform(){
...
// Mutación uniforme: se elige un valor de probabilidad de mutacion mutProb entre mutProbLow y mutProbHigh
// cada elemento es mutado con una probabilidad mutProb
void TwoDPack_MultiObjectivized::mutationUniformValid(){
	double mutProb = mutProbLow + (mutProbHigh - mutProbLow) * ((double)(rand()) / RAND_MAX);
	for (int i = 0; i < twoDPackingProblem->getXSize(); i++){
		for (int j = 0; j < twoDPackingProblem->getYSize(); j++){
			if((1.0*rand()/(RAND_MAX+1.0)) < mutProb){
...
void TwoDPack_MultiObjectivized::mutationWindow() {
...
void TwoDPack_MultiObjectivized::dependentMutation( double pm ) {
	switch(typeMutation){
		case 0:
			mutationUniformValid();
			break;
...
void TwoDPack_MultiObjectivized::subStringCrossover(TwoDPack_MultiObjectivized *ind){
	int columna = (int)((twoDPackingProblem->getXSize()) * ((double)(rand()/(RAND_MAX + 1.0))));
...
void TwoDPack_MultiObjectivized::dependentCrossover (Individual *ind){
	subStringCrossover((TwoDPack_MultiObjectivized *) ind);	
...
Individual* TwoDPack_MultiObjectivized::clone() const{
	TwoDPack_MultiObjectivized *newInd = new TwoDPack_MultiObjectivized();
...
const TwoDPackingProblem* TwoDPack_MultiObjectivized::twoDPackingProblem = NULL;
vector< pair<int, int> * >* TwoDPack_MultiObjectivized::neighboursToExplore = NULL;
vector< pair<Position, Position> * >* TwoDPack_MultiObjectivized::partnersToExplore = NULL;
int TwoDPack_MultiObjectivized::nInds = 0;
