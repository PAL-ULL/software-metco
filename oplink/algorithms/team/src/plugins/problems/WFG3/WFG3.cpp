...
bool WFG3::init (const vector<string> &params) {
	// Comprueba los parametros del problema
	if (params.size() != 1) {
		cout << "Error WFG3: Incorrect parameters (number_of_objectives)" << endl;
		return false;
	}
...
void WFG3::evaluate (void) {
  ...
  vector<double> y;
	...
	y = WFG3_t1(y, k);
	y = WFG3_t2(y, k);
	y = WFG3_t3(y, k, getNumberOfObj());

	// Calcula los valores de las funciones objetivo
	vector<double> result = WFG3_shape(y);
