#ifndef __PROBLEMRND_COMPLETE_EVALUATIONS_H__
#define __PROBLEMRND_COMPLETE_EVALUATIONS_H__
...
class ProblemRND_CompleteEvaluations {
	public:
		// Constructor para un problema con bs omni-direccionales (covertura circular)
		ProblemRND_CompleteEvaluations(const string &fich_dim, const string &fich_inst, const int radio);
		ProblemRND_CompleteEvaluations(const string &fich_inst);
		...
		unsigned int calculateCoverage(RND_CompleteEvaluations *ind, unsigned int &usedBs, vector< vector<int> > *grid);

		// Inicializacion de la matriz del conjunto de puntos que puede cubrir cada antena 
		void calculateBsCoveredPoints();

		void calculateAntenas(RND_CompleteEvaluations *ind, vector< vector<int> > *posiciones);
