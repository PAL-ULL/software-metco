...
// Problema a resolver
ProblemRND *RND_Mono::problem = NULL;	
map< int, vector< vector <int> > > RND_Mono::bsRegion;
vector< vector< vector< vector <int> > > > RND_Mono::RegionBs;
vector< vector<int> > RND_Mono::posiciones;
string RND_Mono::instanceFile;
unsigned int RND_Mono::type_instance;
vector<string> RND_Mono::crossParamNames;
vector<string> RND_Mono::crossParamValues;
...
bool RND_Mono::init(const vector<string> &params) {
	// Check for the problem parameters
	if (params.size() < 1){
		cout << "Error problem RND_Mono: type_fich other_parameters" << endl;
	}
		
	int currentArg = 0;
	if (atoi(params[0].c_str()) == 1){
		if (params.size() < 5) {
			cout << "Error problem RND_Mono: type_fich fich_dim fich_inst radio type_crossover Other_parameters" << endl;
			return false;
		}
		currentArg = 4;
	} else if (atoi(params[0].c_str()) == 2){
		if (params.size() < 3){
			cout << "Error problem RND_Mono: type_fich fich_inst type_crossover Other_parameters" << endl;
			return false;
		}
		currentArg = 2; 
	}
	...
	if ((type_crossover == CROSSOVER_OPC) && (params.size() != currentArg + 1)){//Crossover OPC
		cout << "Error problem RND_Mono with crossover OPC: type_fich fich_dim fich_inst radio type_crossover" << endl;
		return false;
	} else if ((type_crossover == CROSSOVER_GEOGRAPHIC_CIRCULAR) && (params.size() != currentArg + 2)){//Crossover geographic Circular
		cout << "Error problem RND_Mono with crossover Geographic Circular: type_fich fich_dim fich_inst radio type_crossover radio" << endl;
		return false;
	} else if ((type_crossover == CROSSOVER_GEOGRAPHIC_RECTANGULAR) && (params.size() != currentArg + 4)){
		cout << "Error problem RND_Mono with crossover Geographic Rectangular: type_fich fich_dim fich_inst radio type_crossover type_file Min_dim Max_dim" << endl;
		return false;
	} 
	if ((type_crossover != CROSSOVER_OPC) && (type_crossover != CROSSOVER_GEOGRAPHIC_CIRCULAR) && (type_crossover != CROSSOVER_GEOGRAPHIC_RECTANGULAR)){
		cout << "Error problem RND_Mono with unknown crossover id" << endl;
		return false;
	}
...
void RND_Mono::restart(){
  
  // INICIO: Heuristica de inicializacion de individuos
	
  /*int uniform_value= (int)(11.0 * rand() / (RAND_MAX + 1.0));
	unsigned int windowRadius = radio_region - (unsigned int)(uniform_value);
  unsigned int windowSize = windowRadius * 2 + 1;

	// Maxima distancia entre dos puntos de una misma ventana
	// unsigned int maxD = 2 * windowRadius * windowRadius; 
	// Maxima distancia entre dos puntos de una misma grid
	unsigned int maxD = (problem->getGridSizeX() * problem->getGridSizeX()) + (problem->getGridSizeY() * problem->getGridSizeY());

	
  // Elegir las BS que se utilizaran en la solucion inicial
	for (unsigned int i = 0; i < problem->getGridSizeX(); i += windowSize) {
		for (unsigned int j = 0; j < problem->getGridSizeY(); j += windowSize) {
      uniform_value= (int)(6.0 * rand() / (RAND_MAX + 1.0));
			unsigned int windowCenterX = i + windowRadius + (unsigned int)(uniform_value);
      uniform_value= (int)(6.0 * rand() / (RAND_MAX + 1.0));
			unsigned int windowCenterY = j + windowRadius + (unsigned int)(uniform_value);
			unsigned int minD = maxD;
			unsigned int minBs = problem->getBsLocationsSize();
			for (unsigned int k = 0; k < problem->getBsLocationsSize(); k++) {
				unsigned int dX = problem->getbsLocationsX(k) - windowCenterX;
				unsigned int dY = problem->getbsLocationsY(k) - windowCenterY;
				dX = dX * dX;
				dY = dY * dY;
				unsigned int d = dX + dY;

				if ((d < minD) && (getVar(k) != 1) && ((rand()/(RAND_MAX * 1.0)) < 0.8)) {
					if (minBs < problem->getBsLocationsSize()) {
            setVar(minBs, 0);
					}
          setVar(k, 1);
					minD = d;
  ...
  // FIN: Heuristica de inicializacion de individuos
...
unsigned int RND_Mono::getOptDirection(const int i) const{ 
	return MAXIMIZE;
}

// Clonacion
Individual *RND_Mono::clone() const{
	
	RND_Mono *nuevo = new RND_Mono();
	nuevo->grid = grid;
...
void RND_Mono::calculateRegionRect(){
...
unsigned int RND_Mono::getNeighbourhoodSize() const {
...
Individual *RND_Mono::getNeighbour(const int i) const {

	// Crear un nuevo individuo para trabajar sobre el
	RND_Mono *newind = (RND_Mono*)internalClone();
...
void RND_Mono::getNeighbourObjectives(const int i, vector<double> &objectives) const {
	
	double obj1;
	int valorgrid;
	
	if ((int)getVar(i) == 1){ //Si la antena esta a uno
		objectives[0] = usedBs - 1;  // Numero de antenas
		// Se recalcula el cubrimiento
		double obj = coveredPositions;  // Valor del objectivo 1
	...
	/*for (int i = 0; i < objectives.size();i++){
		cout<<"RND_Mono "<<objectives[i]<<endl;
	}*/
}
...
void RND_Mono::removeBs(const unsigned int bsId) {
...
void RND_Mono::addBs(const unsigned int bsId) {
...
void RND_Mono::dependentMutation(double pm) {
...
void RND_Mono::crossover_opc(Individual* ind) {
	int nvars = getNumberOfVar();
	...
	if (vcross >= (nvars/2)) {
		...
		for (unsigned int i = (vcross+1); i < nvars; i++) {
			...
			if ((tmp2 == 1) && (tmp1 == 0)){
				((RND_Mono*)(ind))->addBs(i);
				removeBs(i);
			} else if ((tmp2 == 0) && (tmp1 == 1)){
				((RND_Mono*)(ind))->removeBs(i);
				addBs(i);
			}
		}
	}
...
//
void RND_Mono::crossover_geographic_circular(Individual* ind) {
	int nvars = getNumberOfVar();
	...
	for (unsigned int i = 0; i < bsRegion[radio_region][bs_geographic].size(); i++) {
		...
		if ((tmp2 == 1) && (tmp1 == 0)){
			((RND_Mono*)(ind))->addBs(bsRegion[radio_region][bs_geographic][i]);
			removeBs(bsRegion[radio_region][bs_geographic][i]);
		} else if ((tmp2 == 0) && (tmp1 == 1)){
			((RND_Mono*)(ind))->removeBs(bsRegion[radio_region][bs_geographic][i]);
			addBs(bsRegion[radio_region][bs_geographic][i]);
		}
	}
}

void RND_Mono::crossover_geographic_rectangular(Individual* ind){
	...
	for (unsigned int i = 0; i < antenas.size(); i++){
		...
		if ((tmp2 == 1) && (tmp1 == 0)){
			((RND_Mono*)(ind))->addBs(antenas[i]);
			removeBs(antenas[i]);
		} else if ((tmp2 == 0) && (tmp1 == 1)){
			((RND_Mono*)(ind))->removeBs(antenas[i]);
			addBs(antenas[i]);
		}
	}
...
void RND_Mono::updateFitness(){
	setObj(0, ((double)(coveredPositions)/problem->getGridSizeX() / problem->getGridSizeY() * 100) * ((double)(coveredPositions)/problem->getGridSizeX() / problem->getGridSizeY() * 100) / usedBs);
}

void RND_Mono::dependentCrossover(Individual* ind) {
...
ProblemRND::ProblemRND(const string &fich_dim, const string &fich_inst, const int radio) {
...
ProblemRND::ProblemRND(const string &fich_inst) {
	readInstanceFile(fich_inst);
	//Inicializar la matriz de puntos cubiertos por cada una de las Bs
  calculateBsCoveredPoints();
}

void ProblemRND::readDimFile(const string &fich_dim){
...
//...
//x_NSites Y_NSites
void ProblemRND::readInstanceFile(const string &fich_inst){
...
void ProblemRND::readOldInstanceFile(const string &fich_inst){
...
void ProblemRND::calculateBsCoveredPoints() {
...
unsigned int ProblemRND::calculateCoverage(RND_Mono *rndInd, unsigned int &usedBs, vector< vector<int> > *grid) {
...
void ProblemRND::calculateMaxCoverages() {
...
//Info de la mutacion
string RND_Mono::getMutationName() const{
	return "BitFlipMutation";
}

//Info del crossover
string RND_Mono::getCrossoverName() const{
	if (type_crossover == 0){
...
int RND_Mono::getCrossoverNumberOfParameters() const{
	return crossParamNames.size();
}

string RND_Mono::getCrossoverParameterName(int i) const{
	return crossParamNames[i];
}

string RND_Mono::getCrossoverParameterValue(int i) const{
	return crossParamValues[i];
}

//Info de los objetivos
string RND_Mono::getObjName(int i) const{
	if (i == 0)
		return "fitness";
}
		
void RND_Mono::maintainOnlyObj(){
