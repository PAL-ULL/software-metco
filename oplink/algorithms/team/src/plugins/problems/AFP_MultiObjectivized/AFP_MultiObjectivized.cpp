...
#include "AFP_MultiObjectivized.h"
...
bool AFP_MultiObjectivized::init (const vector<string> &params){
	//Check parameters
	if (params.size() < 3){
...
void AFP_MultiObjectivized::dependentMutation(double pm){
	switch (mutationType){
		case MUTATION_UNIFORM:
			mutationUniform(pm);
			break;
...
void AFP_MultiObjectivized::dependentCrossover(Individual *ind) {
	int trxRandom;
		...
		case CROSSOVER_INTERFERED:
  		trxRandom = (int) ((double)(ind->getNumberOfVar()) * rand() / (RAND_MAX + 1.0));
	 		for (int i = 0; i < ind->getNumberOfVar(); i++) {
      		...
      		((AFP_MultiObjectivized*)ind)->removeAssignTRX(i, tmp);
    		}
  		}
			break;
		case CROSSOVER_UNIFORM:
			for (unsigned int i = 0; i < getNumberOfVar(); i++){
				if (rand() > (RAND_MAX / 2)){
					...
					((AFP_MultiObjectivized*)ind)->removeAssignTRX(i, getVar(i));
...
void AFP_MultiObjectivized::mutationUniform(double pm){
...
void AFP_MultiObjectivized::mutationRename(double pm){
...
// At the end, performs a localSearch
void AFP_MultiObjectivized::mutationNeighbours(double pm){
	MiSetInt *newSectorsToTry = new MiSetInt(problem->getHighestSector());
...
void AFP_MultiObjectivized::calculateInfluencedTrxs(){
...
void AFP_MultiObjectivized::removeAssignTRX(int trx, int freq){
	int dif;
	...
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		...
		if ((int)getVar(iter->first) == (int)getVar(trx)){
			...
			if (indexObj == 2){
				setObj(0, getObj(0) - getCacheData(iter->second) / 2);
		...
		if ((int)getVar(iter->first) == freq){
			...
			if (indexObj == 2){
				setObj(0, getObj(0) + (*coCostBoth)[trx][iter->first] / 2);
        ...
        setCacheData(iter->second, 0);
			...
			if ((dif == 1) || (dif == -1)){
				...
				if (indexObj == 2){
					setObj(0, getObj(0) + (*adjCostBoth)[trx][iter->first] / 2);
...
void AFP_MultiObjectivized::removeAssignTRX2(int trx, int freq, set<int> &interfPrevious){
	int dif;
...
void AFP_MultiObjectivized::randomAssignment(){
	for (unsigned int i = 0; i < (*trxs).size(); i++){
		vector<int> *trxFreq = (*trxs)[i].validFrequencies;
...
void AFP_MultiObjectivized::restart(){
	randomAssignment();
...
Individual *AFP_MultiObjectivized::clone() const{ 
	AFP_MultiObjectivized *newInd = new AFP_MultiObjectivized();
...
void AFP_MultiObjectivized::cloned(){
	//Calculamos la tabla cache
	setCacheDataSize(problem->getIndexesInTable());
	problem->completeEvaluation(this);
}

void AFP_MultiObjectivized::deserialized(){
	//Calculamos la tabla cache
	setCacheDataSize(problem->getIndexesInTable());
	problem->completeEvaluation(this);
}
...
*/
void AFP_MultiObjectivized::localSearch(){
...
void AFP_MultiObjectivized::improveSectors(MiSetInt *newSectorsToTry){
...
bool AFP_MultiObjectivized::improveSector(int selectedSector){
...
bool AFP_MultiObjectivized::assignTrxToFrequency(set<int> &notAssignedTrxs, int freq, multiset<int> &possibleFrequencies, vector<int> &assignedTrxs){
...
void AFP_MultiObjectivized::assignTRX(int trx, int freq){
	int dif;
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		if ((int)getVar(iter->first) == freq){
			int indexObj = problem->getTrx2Obj(trx, iter->first, 1);
...
void AFP_MultiObjectivized::removeTrx(int trx) {
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		//setObj(0, getObj(0) - getCacheData(iter->second));
		if ((int)getVar(iter->first) == (int)getVar(trx)){
			int indexObj = problem->getTrx2Obj(trx, iter->first, 1);
...
void AFP_MultiObjectivized::getAssignmentCost(int trx, vector<double> &assignmentCosts) const{
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
...
void AFP_MultiObjectivized::print(ostream &os) const {
...
void AFP_MultiObjectivized::addVictimInterfererCostValue(int index, double cost){
	setCacheData(index, getCacheData(index) + cost);
}

// Print intialization error
void AFP_MultiObjectivized::initialization_error(){
...
// value
void AFP_MultiObjectivized::evaluate(){
}

//Static members of AFP class
AFPProblem *AFP_MultiObjectivized::problem = NULL;
vector<PairSectorCost> *AFP_MultiObjectivized::sortCosts = NULL;
vector<int> *AFP_MultiObjectivized::dependentSectors;
vector<int> *AFP_MultiObjectivized::independentSectors;
vector< vector<int> > *AFP_MultiObjectivized::sectors2trxs;
vector<TRX> *AFP_MultiObjectivized::trxs;
vector< vector <double> > *AFP_MultiObjectivized::adjCostBoth;
vector< vector <double> > *AFP_MultiObjectivized::coCostBoth;
vector<map <int, int> > *AFP_MultiObjectivized::victimOrInterfererTrxCost;
vector< set <int> > *AFP_MultiObjectivized::sectorsInterfereOrVictim;
vector< set <int> > AFP_MultiObjectivized::directInfluencedTrxs;
bool AFP_MultiObjectivized::origCodeLoaded = false;
int AFP_MultiObjectivized::firstInitialization = 1;
...
void AFPProblem::completeEvaluation(AFP_MultiObjectivized *ind){
	int i;
	double cost = 0;
	ind->setObj(0, 0);
	ind->setObj(1, 0);
	...
	for (int i = 0; i < (int)trxs.size(); i++){//foreach TRX
		for (vector<int>::iterator iterk = sectors2trxs[trxs[i].sectorID].begin(); iterk != sectors2trxs[trxs[i].sectorID].end(); iterk++){//trxs in the sector 
			...
			if ((i != *iterk) && ((freqi) == (int)(ind->getVar(*iterk))) ){
				...
				ind->setObj(0, ind->getObj(0) +	(*coCost)[i][*iterk] / 2.0);
...
void AFPProblem::generateCostTrxMatrix(){
  ...
  for (int i = 0; i < orderedTrx.size(); i++) {
    if (trx2sector[orderedTrx[i].trxA] != trx2sector[orderedTrx[i].trxB]) {
      trx2Obj[orderedTrx[i].trxA][orderedTrx[i].trxB][orderedTrx[i].type] = (counter % 2);
      trx2Obj[orderedTrx[i].trxB][orderedTrx[i].trxA][orderedTrx[i].type] = (counter % 2);
      counter++;
    }
    else {
      trx2Obj[orderedTrx[i].trxA][orderedTrx[i].trxB][orderedTrx[i].type] = 2;
      trx2Obj[orderedTrx[i].trxB][orderedTrx[i].trxA][orderedTrx[i].type] = 2;
    }
  }
