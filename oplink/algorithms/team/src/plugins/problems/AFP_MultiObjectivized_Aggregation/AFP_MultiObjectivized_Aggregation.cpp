...
#include "AFP_MultiObjectivized_Aggregation.h"
...
bool AFP_MultiObjectivized_Aggregation::init (const vector<string> &params){
	//Check parameters
	if (params.size() < 3){
...
void AFP_MultiObjectivized_Aggregation::dependentMutation(double pm){
	switch (mutationType){
		case MUTATION_UNIFORM:
			mutationUniform(pm);
			break;
...
void AFP_MultiObjectivized_Aggregation::dependentCrossover(Individual *ind) {
	int trxRandom;
		...
		case CROSSOVER_INTERFERED:
  		trxRandom = (int) ((double)(ind->getNumberOfVar()) * rand() / (RAND_MAX + 1.0));
	 		for (int i = 0; i < ind->getNumberOfVar(); i++) {
      		...
      		((AFP_MultiObjectivized_Aggregation*)ind)->removeAssignTRX(i, tmp);
    		}
  		}
			break;
		case CROSSOVER_UNIFORM:
			for (unsigned int i = 0; i < getNumberOfVar(); i++){
				if (rand() > (RAND_MAX / 2)){
					...
					((AFP_MultiObjectivized_Aggregation*)ind)->removeAssignTRX(i, getVar(i));
...
void AFP_MultiObjectivized_Aggregation::mutationUniform(double pm){
...
void AFP_MultiObjectivized_Aggregation::mutationRename(double pm){
...
// At the end, performs a localSearch
void AFP_MultiObjectivized_Aggregation::mutationNeighbours(double pm){
	MiSetInt *newSectorsToTry = new MiSetInt(problem->getHighestSector());
...
void AFP_MultiObjectivized_Aggregation::calculateInfluencedTrxs(){
...
void AFP_MultiObjectivized_Aggregation::removeAssignTRX(int trx, int freq){
	int dif;
	...
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		...
		if ((int)getVar(iter->first) == (int)getVar(trx)){
			int indexObj = problem->getTrx2Obj(trx, iter->first, 1);
			if (indexObj == 2 + 1){
				setObj(1, getObj(1) - getCacheData(iter->second) / 2);
				//setObj(2, getObj(2) - getCacheData(iter->second) / 2);
			} else if (indexObj == 1) {
				setObj(indexObj, getObj(indexObj) - getCacheData(iter->second));
			}
		} else {
			dif = (int)getVar(iter->first) - (int)getVar(trx);
			if ((dif == 1) || (dif == -1)){
				int indexObj = problem->getTrx2Obj(trx, iter->first, 0);
				if (indexObj == 2 + 1){
					setObj(1, getObj(1) - getCacheData(iter->second) / 2);
					//setObj(2, getObj(2) - getCacheData(iter->second) / 2);
			  } else if (indexObj == 1) {
		...
		if ((int)getVar(iter->first) == freq){
			...
			if (indexObj == 2 + 1){
				setObj(1, getObj(1) + (*coCostBoth)[trx][iter->first] / 2);
				//setObj(2, getObj(2) + (*coCostBoth)[trx][iter->first] / 2);
				//setCacheData(iter->second, (*coCostBoth)[trx][iter->first]);
			} else if (indexObj == 1) {
				setObj(indexObj, getObj(indexObj) + (*coCostBoth)[trx][iter->first]);
				//setCacheData(iter->second, (*coCostBoth)[trx][iter->first]);
			} 
      //else {
      ...
      //}
			...
			if ((dif == 1) || (dif == -1)){
				...
				if (indexObj == 2 + 1){
					setObj(1, getObj(1) + (*adjCostBoth)[trx][iter->first] / 2);
					//setObj(2, getObj(2) + (*adjCostBoth)[trx][iter->first] / 2);
					//setCacheData(iter->second, (*adjCostBoth)[trx][iter->first]);
				} else if (indexObj == 1) {
					setObj(indexObj, getObj(indexObj) + (*adjCostBoth)[trx][iter->first]);
					//setCacheData(iter->second, (*adjCostBoth)[trx][iter->first]);
				} 
        //else {
        // setCacheData(iter->second, 0);
...
void AFP_MultiObjectivized_Aggregation::removeAssignTRX2(int trx, int freq, set<int> &interfPrevious){
	int dif;
...
void AFP_MultiObjectivized_Aggregation::randomAssignment(){
	for (unsigned int i = 0; i < (*trxs).size(); i++){
		vector<int> *trxFreq = (*trxs)[i].validFrequencies;
...
void AFP_MultiObjectivized_Aggregation::restart(){
	randomAssignment();
...
Individual *AFP_MultiObjectivized_Aggregation::clone() const{ 
	AFP_MultiObjectivized_Aggregation *newInd = new AFP_MultiObjectivized_Aggregation();
...
void AFP_MultiObjectivized_Aggregation::cloned(){
	//Calculamos la tabla cache
	setCacheDataSize(problem->getIndexesInTable());
	problem->completeEvaluation(this);
}

void AFP_MultiObjectivized_Aggregation::deserialized(){
	//Calculamos la tabla cache
	setCacheDataSize(problem->getIndexesInTable());
	problem->completeEvaluation(this);
}
...
*/
void AFP_MultiObjectivized_Aggregation::localSearch(){
...
void AFP_MultiObjectivized_Aggregation::improveSectors(MiSetInt *newSectorsToTry){
...
bool AFP_MultiObjectivized_Aggregation::improveSector(int selectedSector){
...
bool AFP_MultiObjectivized_Aggregation::assignTrxToFrequency(set<int> &notAssignedTrxs, int freq, multiset<int> &possibleFrequencies, vector<int> &assignedTrxs){
...
void AFP_MultiObjectivized_Aggregation::assignTRX(int trx, int freq){
	int dif;
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		if ((int)getVar(iter->first) == freq){
			int indexObj = problem->getTrx2Obj(trx, iter->first, 1);
...
void AFP_MultiObjectivized_Aggregation::removeTrx(int trx) {
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
		setObj(0, getObj(0) - getCacheData(iter->second));
		if ((int)getVar(iter->first) == (int)getVar(trx)){
			int indexObj = problem->getTrx2Obj(trx, iter->first, 1);
			...
			int dif = (int)getVar(iter->first) - (int)getVar(trx);
			if ((dif == 1) || (dif == -1)){
				int indexObj = problem->getTrx2Obj(trx, iter->first, 0);
				if (indexObj == 2 + 1){
					setObj(1, getObj(1) - getCacheData(iter->second) / 2);
...
void AFP_MultiObjectivized_Aggregation::getAssignmentCost(int trx, vector<double> &assignmentCosts) const{
	for (map<int, int>::iterator iter = (*victimOrInterfererTrxCost)[trx].begin(); iter != (*victimOrInterfererTrxCost)[trx].end(); iter++){
...
void AFP_MultiObjectivized_Aggregation::print(ostream &os) const {
...
void AFP_MultiObjectivized_Aggregation::addVictimInterfererCostValue(int index, double cost){
	setCacheData(index, getCacheData(index) + cost);
}

// Print intialization error
void AFP_MultiObjectivized_Aggregation::initialization_error(){
...
// value
void AFP_MultiObjectivized_Aggregation::evaluate(){
}

//Static members of AFP class
AFPProblem *AFP_MultiObjectivized_Aggregation::problem = NULL;
vector<PairSectorCost> *AFP_MultiObjectivized_Aggregation::sortCosts = NULL;
vector<int> *AFP_MultiObjectivized_Aggregation::dependentSectors;
vector<int> *AFP_MultiObjectivized_Aggregation::independentSectors;
vector< vector<int> > *AFP_MultiObjectivized_Aggregation::sectors2trxs;
vector<TRX> *AFP_MultiObjectivized_Aggregation::trxs;
vector< vector <double> > *AFP_MultiObjectivized_Aggregation::adjCostBoth;
vector< vector <double> > *AFP_MultiObjectivized_Aggregation::coCostBoth;
vector<map <int, int> > *AFP_MultiObjectivized_Aggregation::victimOrInterfererTrxCost;
vector< set <int> > *AFP_MultiObjectivized_Aggregation::sectorsInterfereOrVictim;
vector< set <int> > AFP_MultiObjectivized_Aggregation::directInfluencedTrxs;
bool AFP_MultiObjectivized_Aggregation::origCodeLoaded = false;
int AFP_MultiObjectivized_Aggregation::firstInitialization = 1;
...
void AFPProblem::completeEvaluation(AFP_MultiObjectivized_Aggregation *ind){
	int i;
	double cost = 0;
	ind->setObj(0, 0);
	ind->setObj(1, 0);
			...
			for(vector<int>::iterator victimTrx = sectors2trxs[victimSector].begin(); victimTrx != sectors2trxs[victimSector].end(); victimTrx ++) { //Each trx in the victim sector
				for (vector<int>::iterator interfererTrx = sectors2trxs[interfererSector].begin(); interfererTrx != sectors2trxs[interfererSector].end(); interfererTrx ++) { //Each trx in the interferer sector
					...
					if (abs(diff) == 1){
						...
						int indexObj = getTrx2Obj(*victimTrx, *interfererTrx, 0);
            if (indexObj == 1)
						  ind->setObj(indexObj, ind->getObj(indexObj) + (*adjCost)[*victimTrx][*interfererTrx]);
					} else if (diff == 0){
						...
						int indexObj = getTrx2Obj(*victimTrx, *interfererTrx, 1);
            if (indexObj == 1)
						  ind->setObj(indexObj, ind->getObj(indexObj) + (*coCost)[*victimTrx][*interfererTrx]);
					}
				}
		  } 
    }
	...
	for (int i = 0; i < (int)trxs.size(); i++){//foreach TRX
		for (vector<int>::iterator iterk = sectors2trxs[trxs[i].sectorID].begin(); iterk != sectors2trxs[trxs[i].sectorID].end(); iterk++){//trxs in the sector 
			...
			if ((i != *iterk) && ((freqi) == (int)(ind->getVar(*iterk))) ){
				...
				ind->setObj(1, ind->getObj(1) +	(*coCost)[i][*iterk] / 2.0);
				//ind->setObj(2, ind->getObj(2) +	(*coCost)[i][*iterk] / 2.0);
...
// Comparacion por valor de aparicion de parejas
bool cmpTrxValueStruct (const TrxValueStruct &a, const TrxValueStruct &b) {
...
void AFPProblem::generateCostTrxMatrix(){
  ...
  for (map<int, map<int, pair<double,double> > >::iterator victim = interferenceMatrix.begin(); victim != interferenceMatrix.end(); victim++ ) { //for each victim sector
	...
	//Init assignment of obj
	trx2Obj.insert(trx2Obj.begin(), trxs.size(), vector < vector < int > > (trxs.size(), vector <int>( 2, -20 ) ) );
  //calculates the ordered costs table
  for (int i = 0; i < trxs.size(); i++) {
    for (int j = i + 1; j < trxs.size(); j++) {
      if ((*adjCostBoth)[i][j] > 0) {
        TrxValueStruct newTrxEntry;
        newTrxEntry.trxA = i;
        newTrxEntry.trxB = j;
        newTrxEntry.type = 0;
        newTrxEntry.value = (*adjCostBoth)[i][j];
        orderedTrx.push_back(newTrxEntry);
      }
      if ((*coCostBoth)[i][j] > 0) {
        TrxValueStruct newTrxEntry;
        newTrxEntry.trxA = i;
        newTrxEntry.trxB = j;
        newTrxEntry.type = 1;
        newTrxEntry.value = (*coCostBoth)[i][j];
        orderedTrx.push_back(newTrxEntry);
      }
    }
  }
  
  sort(orderedTrx.begin(), orderedTrx.end(), cmpTrxValueStruct);

  int counter = 0;

  for (int i = 0; i < orderedTrx.size(); i++) {
    if (trx2sector[orderedTrx[i].trxA] != trx2sector[orderedTrx[i].trxB]) {
      trx2Obj[orderedTrx[i].trxA][orderedTrx[i].trxB][orderedTrx[i].type] = (counter % 2) + 1;
      trx2Obj[orderedTrx[i].trxB][orderedTrx[i].trxA][orderedTrx[i].type] = (counter % 2) + 1;
      counter++;
    }
    else {
      trx2Obj[orderedTrx[i].trxA][orderedTrx[i].trxB][orderedTrx[i].type] = 2 + 1;
      trx2Obj[orderedTrx[i].trxB][orderedTrx[i].trxA][orderedTrx[i].type] = 2 + 1;
    }
  }
...
void AFPProblem::loadTrxs(const string &filename){
	...
	while (!file.eof()) {
		...
		//read sector ID
 		file >> t->sectorID;
    if (t->trxID != trx2sector.size()) {
      cout << "Internal error: TRX not ordered in file " << filename << endl;
      exit(-1);
    }
    trx2sector.push_back(t->sectorID);
