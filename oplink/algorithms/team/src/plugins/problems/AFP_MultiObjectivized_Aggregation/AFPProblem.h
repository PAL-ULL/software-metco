...
class AFP_MultiObjectivized_Aggregation;

struct TrxValueStruct {
  int trxA;
  int trxB;
  int type;
  double value;
...
class AFPProblem {
	public:
		...
		void completeEvaluation(AFP_MultiObjectivized_Aggregation *ind);
		...
		inline int getTrx2Obj(int trxA, int trxB, int type) const { return trx2Obj[trxA][trxB][type]; }
    ...
    vector<int>                               trx2sector          ; //map trxs to sectors
    ...
    // Multiobjectivization attributes
    vector< TrxValueStruct > orderedTrx; // Trx ordered by value
    vector< vector < vector <int> > > trx2Obj; // Specifies which objective is considered for the trx
