...
bool WFG8::init (const vector<string> &params) {
	// Comprueba los parametros del problema
	if (params.size() != 1) {
		cout << "Error WFG8: Incorrect parameters (number_of_objectives)" << endl;
		return false;
	}
	...
	if (nObj < 2) {
	  cout << "Error WFG8: Parameter number_of_objectives must be greater than 2" << endl;
		return false;
	}
...
void WFG8::evaluate (void) {
  ...
  vector<double> y;
	...
	y = WFG8_t1(y, k);
	y = WFG8_t2(y, k);
	y = WFG8_t3(y, k, getNumberOfObj());

	// Calcula los valores de las funciones objetivo
	vector<double> result = WFG8_shape(y);
