 ...
 *         individuo. Puede llamar a alguno de los m�todos de mutaci�n de la clase
 *         padre o implementar su propio m�todo.
 *     
 ...
 *            - Fijar los rangos de valores v�lidos (minimos y m�ximos) para cada
 *              gen. Los valores minimos se pueden establecer de 2 formas:
 *                * LLamando al metodo setMinimumValues(const double minValues[]),
 *                  pas�ndole un array que contiene el valor m�nimo para cada 
 *                  objetivo.
 *                * Llamando sucesivamente al metodo 
 *                setNextMinimumValue(const double value) (una vez por cada objetivo)
 *                pas�ndole cada vez el valor m�nimo correspondiente al objetivo.
 *              Los valores m�ximos se tratan de la misma forma llamando a los m�todos
 *              setMaximumValues o setNextMaximumValue
 *            - Fijar el n�mero de datos adicionales del individuo, que quieren que
 *            sean sean migrados y clonados de forma autom�tica. S�lo hace faltar fijarlo
 *            si es distinto de 0. Se realiza llamando al m�todo setMigrationDataSize.
 *          Retorna un valor booleano:
 *          - True: la inicializaci�n fue correcta.
 *          - False: error en los par�metros pasados. Se abortar� la ejecuci�n.
 *          En los casos en los que se retorne false, se deber� escribir un error
 *          en cerr
 *  
 ...
 *          Este m�todo sirve para realizar la clonaci�n de un individuo. El usuario
 *          debe crear un individuo de la clase hija y devolverlo.
 *          Los genes, valores objetivos y par�metros compartidos son copiados
 *          de forma autom�tica.
 *   
 *
 ...
 *           Este m�todo por defecto imprime los valores de las variables del individuo
 *           seguido de los valores de los objetivos.
 *           En caso de querer mostrar por pantalla alg�n dato m�s del individuo o
 *           en otro formato distinto, se puede sobreescribir este m�todo
 *        
 *        +virtual void restart()
 *           Este m�todo se usa para crear nuevos individuos en la primera poblaci�n. 
 ...
 *           Tras cada paso de generaci�n en la que se use el individuo, este m�todo 
 *           es instanciado. Si es necesario realizar alguna operaci�n adicional tras cada 
 *           generaci�n, se puede hacer en este m�todo.
 *
 *    - Otras notas:
 *        En cualquiera de los m�todo sobrecargados se pueden tener en cuenta los datos
 *        adicionales del individuo (datos a migrar). A estos datos se pueden acceder
 *        con el m�todo:
 *        	double inline getMigrationData(const int i) 
 *        Los valores se pueden fijar con el m�todo:
