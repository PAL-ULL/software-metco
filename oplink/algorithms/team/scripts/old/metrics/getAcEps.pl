...
sub analyze{
	...
	my @values = (2 - (2 -$maxValue)*$valuesPercent[0], 2 - (2 - $maxValue)*$valuesPercent[1], 2 - (2 - $maxValue)*$valuesPercent[2], 2 - (2 - $maxValue)*$valuesPercent[3], 2 - (2 - $maxValue)*$valuesPercent[4]);
	$info{$filename} = {};
	foreach(0..$repetitions){
		push(@percent, $valuesPercent[0]);
		push(@indexPercent, 0);
		push(@actualValue, $values[0]);
		push(@previousEval, 0);
